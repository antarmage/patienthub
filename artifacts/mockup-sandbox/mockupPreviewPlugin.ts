...
export function mockupPreviewPlugin(): Plugin {
  ...
  function generateSource(_components: Array<DiscoveredComponent>): string {
    // Use import.meta.glob so Vite resolves all component paths statically at
    // build time. No filesystem-derived string ever flows into the generated
    // code — the glob pattern is a hard-coded constant (fixes CodeQL
    // js/bad-code-sanitization).
    // Keys produced by Vite are relative to this file (src/.generated/), so
    // they look like "../components/mockups/SubDir/Foo.tsx".
    return [
